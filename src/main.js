...
import { ARABIC_MONTHS, ELEMENT_GUIDE, QUALITY_GUIDE, ZODIAC_DETAILS, ZODIAC_SOURCES, visibleDayCount, isAllowedSourceUrl, formatHijriDate, parseBirthDate, westernDigits, zodiacFromDate, zodiacFromNumber } from './zodiac.js';
...
function letterCountLabel(count) {
  if (!count) return 'بانتظار أول نص';
  if (count === 1) return 'حرف واحد محتسب';
  if (count === 2) return 'حرفان محتسبان';
  if (count <= 10) return `${num(count)} حروف محتسبة`;
  return `${num(count)} حرفًا محتسبًا`;
}
function calculator() {
...
function planetShortTitle(text = '') {
  if (text.startsWith('تعادل كوكبي')) return 'تعادل حرفي';
  if (text.startsWith('توافق كوكبي')) return 'توافق حرفي';
  if (text.startsWith('اختلاف كوكبي')) return 'اختلاف حرفي';
  return text;
}
function nameComparison(a, b) {
  ...
  const dominancePanel = ready ? `<section class="dominance-dashboard" id="comparison-result"><div class="dominance-dashboard-head"><div><span class="eyebrow">النتيجة وفق الحساب التراثي</span><h3>النتيجة</h3></div><span class="comparison-note">نتيجة حسابية وليست حكمًا على قيمة الشخص.</span></div><div class="dominance-match"><article class="dominance-side winner"><span>الغالب حسابيًا</span><b>${esc(winnerName)}</b><small>الباقي من ٩: ${num(winnerProfile.mod9)}</small></article><div class="dominance-arrow" aria-hidden="true"><b>⇄</b><small>مقارنة</small></div><article class="dominance-side loser"><span>المغلوب حسابيًا</span><b>${esc(loserName)}</b><small>الباقي من ٩: ${num(loserProfile.mod9)}</small></article></div><div class="dominance-rule"><b>سبب الحكم</b><p>${comparison.reason}</p></div></section><div class="comparison-insights"><article><span class="insight-number">١</span><div><small>قاعدة الحكم</small><h3>الباقي من القسمة على ٩</h3><p>${comparison.reason}</p></div></article><article><span class="insight-number">٢</span><div><small>علاقة الطبائع</small><h3>${elements.kind}</h3><p>${elements.text}</p></div></article><article><span class="insight-number">٣</span><div><small>المراسلات الحرفية</small><h3>${planetShortTitle(celestial.planet)}</h3><p>${celestial.planet} ${celestial.zodiac}</p></div></article></div>` : '<div class="comparison-placeholder"><b>أدخل الاسمين لإظهار الغالب والمغلوب</b><span>ستظهر النتيجة وسبب الحكم وتفاصيل الطبائع في لوحة واحدة.</span></div>';
  return `<section class="card comparison-workspace" id="name-comparison"><div class="section-heading"><div><h2>الغالب والمغلوب</h2><p class="muted">نتيجة حسابية تراثية تُظهر قاعدة الحكم وتفاصيل الطرفين.</p></div><span class="tag">الباقي من ٩</span></div><div class="comparison-inputs"><label><span>الشخص الأول${state.nameA.trim() ? ' — ' + esc(state.nameA.trim()) : ''}</span><input data-field="nameA" value="${esc(state.nameA)}" placeholder="الاسم"><input data-field="motherA" value="${esc(state.motherA)}" placeholder="اسم الأم (اختياري)"></label><label><span>الشخص الثاني${state.nameB.trim() ? ' — ' + esc(state.nameB.trim()) : ''}</span><input data-field="nameB" value="${esc(state.nameB)}" placeholder="الاسم"><input data-field="motherB" value="${esc(state.motherB)}" placeholder="اسم الأم (اختياري)"></label></div><div class="actions comparison-actions"><label class="role-pick"><span>دور الشخص الأول</span><select data-field="role"><option ${firstRole === 'طالب' ? 'selected' : ''}>طالب</option><option ${firstRole === 'مطلوب' ? 'selected' : ''}>مطلوب</option></select></label><button class="btn" data-clear="names">مسح</button></div>${dominancePanel}${ready ? `<details class="comparison-evidence" data-disclosure="comparison-evidence"><summary><b>تفاصيل الحساب للطرفين</b><span>للمراجعة والاطلاع — اضغط للعرض</span></summary><div class="duel-grid">${comparePersonCard('الشخص الأول', firstRole, state.nameA, state.motherA, a, firstWins)}<div class="versus-mark"><b>↔</b><span>تفاصيل</span></div>${comparePersonCard('الشخص الثاني', secondRole, state.nameB, state.motherB, b, !firstWins)}</div></details><details class="comparison-details" data-disclosure="comparison-details"><summary>عرض التفاصيل الحسابية الكاملة للشخصين</summary><div class="split"><div class="profile-detail"><h3>${esc(state.nameA)}</h3>${profileCard(a)}</div><div class="profile-detail"><h3>${esc(state.nameB)}</h3>${profileCard(b)}</div></div></details>` : ''}</section>`;
}
function names() {
...
function suggestions() {
  ...
  const isFresh = !desired.count && !parentEntries.length;
  const desiredResult = isFresh
    ? '<div class="baby-empty is-start"><b>ابدأ بخطوتين فقط ✦</b><span>١- اكتب اسم الأب أو الأم ٢- جرّب الاسم المرغوب وسنحسب قيمته ونقارنه فورًا.</span></div>'
    : !desired.count
...
function natalZodiacTool() {
  ...
  const hijri = birth.status === 'valid' ? formatHijriDate(birth.day, birth.month, birth.year) : null;
...
function zodiac() {
  ...
  return `<section class="zodiac-hero natal-hero compact-hero"><div><span class="eyebrow">حساب واضح · تفسير رمزي بحدود معلنة</span><h2>الأبراج وخريطة الميلاد</h2><p>أداتان منفصلتان: حساب حرفي تراثي، وخريطة فلكية رمزية.</p><div class="hero-methods"><span>☉ مواضع فلكية</span><span>⌖ توقيت ومكان</span><span>◎ البيوت الكاملة</span></div></div><div class="zodiac-orbit"><span>✦</span>${ZODIAC_DETAILS.map((_, index) => `<i style="--i:${index}"></i>`).join('')}</div></section>
  ...
  ${selectedTool}
  <div class="detail-accordion supporting-details">${progressivePanel('دليل الأبراج','مرجع سريع',`<div class="zodiac-guide">${ZODIAC_DETAILS.map(zodiacGuideCard).join('')}</div>`,full)}${progressivePanel('حدود القراءة','ما هو حسابي وما هو رمزي',boundaries,full)}${progressivePanel('المصادر والمنهج','للتحقق بنفسك',`<div class="source-list">${sources}</div>`,full)}</div>`;
}
function reference() {
  const full = state.detailMode === 'full';
  ...
  return `<section class="reference-intro card"><div><h2>المرجع والمنهج</h2><p class="muted">اختر الباب الذي تحتاجه؛ كل التفاصيل الحسابية محفوظة دون إغراق الصفحة.</p></div><span class="tag">جداول وقيم</span></section><div class="detail-accordion reference-accordion">${progressivePanel('الأبجد المشرقي','قيم الحروف المعتمدة',abjad,full)}${progressivePanel('الطبائع','توزيع الحروف',elements,full)}${progressivePanel('الكواكب','الترتيب المستخدم',planets,full)}${progressivePanel('البروج','الترتيب والعناصر',signs,full)}</div><section class="card reference-tools"><div><small>تطوير</small><b>إبراهيم بن صلاح الدين</b><span>حساب فلكي محلي · البيوت الكاملة · الباقي من ١٢</span></div><div class="actions"><button class="btn primary" data-run-tests>تشغيل الاختبارات الذاتية</button><button class="btn" data-reset-storage>مسح البيانات المحفوظة</button></div></section>`;
}
function render() {
